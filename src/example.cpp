...
//    comm->start();
    ...
    connect(manager,SIGNAL(msgTokenManagerChanged()),this,SLOT(updateManager()));
    ...
    ui->peerName->setText( s->getPeerName());
...
void example::updateModel(int index){
    ...
    model->setItem(index,0,new QStandardItem(t->getPeerName()));
