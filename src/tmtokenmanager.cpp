...
tmTokenManager::tmTokenManager(QObject *parent):baseDevice(parent)
{
    ...
    //信号汇总
    connect(this,SIGNAL(msgStateChanged(quint64)),this,SLOT(managerChanged()));
    connect(this,SIGNAL(msgErrorSet(quint64)),this,SLOT(managerChanged()));
    connect(this,SIGNAL(msgErrorReset(quint64)),this,SLOT(managerChanged()));
    ...
    if( pPeersList.size() ==0 ){
        ...
        connect(self,SIGNAL(msgPeerChanged(QString)),this,SLOT(peerChanged(QString)));
...
int tmTokenManager::start(void){
    ...
    //2、检查self信息
    if( getSelfPeer()->getPeerName().isEmpty() || getSelfPeer()->getPeerIp()== 0 ){
        getSelfPeer()->setState( tmPeer::stateDisable );
...
//      $TMSHB,peer名称(max 255 byte),IP地址(4byte),peer状态(1byte),peer错误状态(1byte),peer令牌优先级(1byte)<CR><LF>
            ...
            if( getSelfPeer()->getPeerName() == temp_name ) continue;//本peer自己发的，丢弃该报文
...
//      $TMTTR,源peer名称(持有token),目标peer名称,超时时间(秒 4byte)<CR><LF>
            ...
            if (source == getSelfPeer()->getPeerName() ){
                //本peer被动交出令牌
                tmPartnerIndex = getPeerIndex( target );
                tokenOrderOut(overtime);
            }
            else if (target == getSelfPeer()->getPeerName()){
                //本peer被动接受令牌
...
//      $TMTTA,源peer名称,目标peer名称<CR><LF>
            ...
            if (target == getSelfPeer()->getPeerName()){
                //本peer主动获取令牌，收到确认
                tmPartnerIndex = getPeerIndex( source );
                tokenTakeInAck();
            }
...
//      $TMTTC,源peer名称,目标peer名称<CR><LF>
            ...
            if (getSelfPeer()->getState() == tmPeer::stateTokenTakeOutPending &&
                    source == getSelfPeer()->getPeerName() ){
                //本peer主动让出交出令牌，收到取消
                tmPartnerIndex = getPeerIndex( target );
                tokenTakeOutCancel();
            }
            else if (getSelfPeer()->getState() == tmPeer::stateTokenTakeInPending &&
                     target == getSelfPeer()->getPeerName() ){
                //本peer主动获取令牌，收到取消
                tmPartnerIndex = getPeerIndex( source );
                tokenTakeInCancel();
            }
...
//      注意：消息的具体内容由其他程序打包送入和取出解析。
        ...
        heartBeat = "$TMMHB,";
        heartBeat += getSelfPeer()->getPeerName().toUtf8();
        heartBeat += ",";
...
//      $TMSHB,peer名称(max 255 byte),IP地址(4byte),peer状态(1byte),peer错误状态(1byte),peer令牌优先级(1byte)<CR><LF>
        ...
        QByteArray heartBeat;
        heartBeat = "$TMSHB,";
        heartBeat += getSelfPeer()->getPeerName().toUtf8();//.toLocal8Bit();
        heartBeat += ",";
        heartBeat += QByteArray::number(getSelfPeer()->getPeerIp(),16);
        heartBeat += ",";
        heartBeat += QByteArray::number(getSelfPeer()->getState(),16);
...
int tmTokenManager::clearPeerInfo(){
    for (int i=0; i<pPeersList.size();i++){
        disconnect(pPeersList.at(i),SIGNAL(msgPeerChanged(QString)),this,SLOT(peerChanged(QString)) );
...
//$TMTTR,源peer名称(持有token),目标peer名称,超时时间(秒 4byte)<CR><LF>
    QByteArray require;
    require = "$TMTTR,";
    require += getSelfPeer()->getPeerName();
    require += ",";
    require += getPartner()->getPeerName();
    require += ",";
    require += overtime>>24;
    require += overtime>>16;
    require += overtime>>8;
...
//      $TMTTC,源peer名称,目标peer名称<CR><LF>
        QByteArray requrie;
        requrie = "$TMTTC,";
        requrie += getSelfPeer()->getPeerName();
        requrie += ",";
        requrie += getPartner()->getPeerName();
        requrie += 0x0D;
        requrie += 0x0A;
...
//      $TMTTA,源peer名称,目标peer名称<CR><LF>
        QByteArray ack;
        ack = "$TMTTA,";
        ack += getSelfPeer()->getPeerName();
        ack += ",";
        ack += getPartner()->getPeerName();
        ack += 0x0D;
        ack += 0x0A;
...
void tmTokenManager::checkPriority(){
    ...
    qint8 aux2 = (qint8)getSelfPeer()->getPeerName().right(1).toLocal8Bit().toInt();
    ...
    while(index<pPeersList.size()){
        ...
        if(self == pPeersList.at(index)->getPeerPriority() &&
                aux2 < (qint8)pPeersList.at(index)->getPeerName().right(1).toLocal8Bit().toInt()){
            self++;
            getSelfPeer()->setPeerPriority(self);
            return;
        }
...
//set & get
QString tmTokenManager::getErrorString(quint64 errorCode, int lang) const{
    switch(errorCode){
        case errorTokenLost:
...
QString tmTokenManager::getStateString(int lang) const{
    switch(getState() ){
        case stateStop:
...
int tmTokenManager::setSelfPeer( tmPeer* newone){
    ...
    //qDebug()<<newone->getPeerName()<<newone->getState()<<newone->getError()<<newone->getPeerPriority()<<newone->getPeerIp();
    getSelfPeer()->update(newone->getPeerName(),newone->getState(),newone->getError(),newone->getPeerPriority(),newone->getPeerIp() );
    return 0;
}
...
int tmTokenManager::setPeer(tmPeer *newone){
    ...
    //qDebug()<<newone->getPeerName()<<newone->getPeerIp()<<newone->getPeerPriority();
    if(newone->getPeerName() != "" && newone->getPeerIp() != 0x0){
        tmPeer* it =0;
        int i;
        for (i=1; i<pPeersList.size();i++){
            it = pPeersList.at(i);
            if( it->getPeerName() == newone->getPeerName()){
                ...
                it->update(newone->getPeerName(),newone->getState(),newone->getError(),newone->getPeerPriority(),newone->getPeerIp());//发现同名节点，替换
                checkPriority();
                checkName();
                return 0;
            }
        ...
        connect(it,SIGNAL(msgPeerChanged(QString)),this,SLOT(peerChanged(QString)));
        it->update(newone->getPeerName(),newone->getState(),newone->getError(),newone->getPeerPriority(),newone->getPeerIp());//未发现同名节点，新建
        checkPriority();
        checkName();
        return 0;
    }
...
tmPeer* tmTokenManager::getPeer(QString& name) const{
    for (int i=1; i<pPeersList.size();i++){
        if( pPeersList.at(i)->getPeerName() == name){
...
void tmTokenManager::managerChanged(){
    emit msgTokenManagerChanged();
}
